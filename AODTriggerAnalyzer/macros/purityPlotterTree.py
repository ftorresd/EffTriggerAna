...
folderName = "purityROOTFiles_V01/"

#   else if(process=="QCDEM15")    return 1279000000.0 *0.0018 *0.6969;
#   else if(process=="QCDEM20")    return 557600000.0  *1.024E-02;
#   else if(process=="QCDEM30")    return 136000000.0  *5.015E-02;
#   else if(process=="QCDEM50")    return 19200000.0   *1.615E-01;
#   else if(process=="QCDEM80")    return 2760000.0    *1.615E-01;
#   else if(process=="QCDEM120")   return 482000.0     *1.468E-01;
#   //
#   else if(process=="QCDMu15")    return 1279000000.0 *2.145E-03;
#   else if(process=="QCDMu20")    return 560000000.0  *6.71E-03;
#   else if(process=="QCDMu30")    return 139000000.0  *1.099E-02;
#   else if(process=="QCDMu50")    return 19800000.0   *2.132E-02;
#   else if(process=="QCDMu80")    return 2730000.0    *4.13E-02;
#   else if(process=="QCDMu120")   return 474000.0     *5.138E-02;
#   //
#   else if(process=="WJets")      return 1.318E+05; 
#   else if(process=="DYToLL")     return 1.90E+04; 
#   else if(process=="DYJetsToLL_M-50")     return 4954; 


processDefinitions = {
	"DYJetsToLL_M-50" : [ROOT.TFile(folderName + "purity_DYJetsToLL_M-50_HLTEff2017_DYJetsToLL_M-50_NoIso_91X_v20.root"), 4954.0],
	"QCD_Pt-80to120_EMEnriched" : [ROOT.TFile(folderName + "purity_QCD_Pt-80to120_EMEnriched_HLTEff2017_QCD_Pt-80to120_EMEnriched_NoIso_91X_v20.root"), 2760000.0*1.615E-01],	
	"QCD_Pt-120to170_EMEnriched" : [ROOT.TFile(folderName + "purity_QCD_Pt-120to170_EMEnriched_HLTEff2017_QCD_Pt-120to170_EMEnriched_NoIso_91X_v20.root"), 482000.0*1.468E-01],
	"QCD_Pt-80to120_MuEnrichedPt5" : [ROOT.TFile(folderName + "purity_QCD_Pt-80to120_MuEnrichedPt5_HLTEff2017_QCD_Pt-80to120_MuEnrichedPt5_NoIso_91X_v20.root"), 2730000.0*4.13E-02],	
	"QCD_Pt-120to170_MuEnrichedPt5" : [ROOT.TFile(folderName + "purity_QCD_Pt-120to170_MuEnrichedPt5_HLTEff2017_QCD_Pt-120to170_MuEnrichedPt5_NoIso_91X_v20.root"), 474000.0*5.138E-02],	
	"WJetsToLNu" : [ROOT.TFile(folderName + "purity_WJetsToLNu_HLTEff2017_WJetsToLNu_NoIso_91X_v20.root"), 1.318E+05],	
	"QCD_Pt-50to80_EMEnriched" : [ROOT.TFile(folderName + "purity_QCD_Pt-50to80_EMEnriched_HLTEff2017_QCD_Pt-50to80_EMEnriched_NoIso_91X_v20.root"), 19200000.0*1.615E-01],	
	"ZToJPsiGamma" : [ROOT.TFile(folderName + "purity_ZToJPsiGamma_HLTEff2017_ZToJPsiGamma_NoIso_90X_v17.root"), 1.881],	
	"QCD_Pt-50to80_MuEnrichedPt5" : [ROOT.TFile(folderName + "purity_QCD_Pt-50to80_MuEnrichedPt5_HLTEff2017_QCD_Pt-50to80_MuEnrichedPt5_NoIso_91X_v20.root"), 19800000.0*2.132E-02],	
}
...
# 	TEfficiency* pEff = new TEfficiency("eff","my efficiency;x;#epsilon",1,0,1.);
# 	pEff->SetUseWeightedEvents();

# 	pEff->FillWeighted(true,20.0, 0.5);
# 	pEff->FillWeighted(false,10.0, 0.5);

# 	cout << pEff->GetEfficiency(1);	
# }

os.system("rm purityPaths.txt")
file = open("purityPaths.txt","w") 

file.write("=== PURITY ===\n")

for hltPath in hltCut:
	print "---- Path: "+hltPath
	h_Num = ROOT.TH1D("h_Num","h_Num",1,0,1.)
	h_Den = ROOT.TH1D("h_Den","h_Den",1,0,1.)
	for process in processDefinitions:
		print "---- Process: "+process
		purFile = processDefinitions[process][0]
		xSec = processDefinitions[process][1]
		# purFile.Print()
		# print purFile
		# print purFile.HLT_Tree
		HLTTree = purFile.HLT_Tree.Get("RECOL1HLTTree")
		nEntries = HLTTree.GetEntries()
		# print nEntries
		denominator = HLTTree.GetEntries(hltCut[hltPath])
		h_Den.Fill(0.5,denominator*xSec/nEntries)
		numerator = HLTTree.GetEntries(recoCut+AND+hltCut[hltPath])
		h_Num.Fill(0.5,numerator*xSec/nEntries)
	pEff = ROOT.TEfficiency(h_Num,h_Den)
	# pEff.SetUseWeightedEvents()
	file.write(hltPath + ": " + str(pEff.GetEfficiency(1)*100.0) + "% [+(" + str(pEff.GetEfficiencyErrorUp(1)*100.0) + ") / -("+ str(pEff.GetEfficiencyErrorLow(1)*100.0) + ")]%\n")


file.close() 

print "\n\n"
os.system("cat purityPaths.txt")
